...
func handle_input(actor, event):
	if event.is_action_pressed(actor.right):
		...
		actor.direction = -1
		actor.velocity.x = in_jump_speed * actor.direction
	if event.is_action_released(actor.right) or \
	event.is_action_released(actor.left):
		actor.velocity.x = 0
...
#		return
	...
	if !actor.has_method("handle_input"):
		return
	if Input.is_action_pressed(actor.right) or \
	Input.is_action_pressed(actor.left):
		if actor.is_on_floor():
			actor.walk()
			return
